...
The **Library grid is sortable** (built 2026-06-24): a **Sort** control on the Library tab orders the grid by **Recent** (newest first, the default), **Oldest**, **Name** (A to Z, case-insensitive), or **Artist** (A to Z by the artist shown on the card, with un-credited pieces last). The choice is a persisted server setting, and the **install sample (the Bouncing OpenObject Logo) stays anchored to the bottom in every order** (§20). This is grid presentation only; the Rotation keeps its own curated order.
