...
_None currently open._ Earlier seams listed here have since shipped (Connected Collections, §8; SVG support, §6) or been dropped as out of scope. The section is kept as a placeholder for future seams; build notes for what shipped live in §20.
...
### 2026-06-30: Golden Lining Safari fix, take two: pre-render the grey, drop the per-frame `filter(GRAY)` (`preRender`)
After `willReadFrequently` failed on iOS (entry below), the working theory sharpened (Matt): it is not the image size (large static images load fine) but the **per-frame full-canvas `filter(GRAY)`** (a whole-canvas pixel read-back every frame) that WebKit tile-corrupts; Chromium does the same readback robustly, which is why every Chromium display (XXL kiosk, Mac Chrome kiosk) is perfect. So the fix removes the per-frame readback entirely. A per-collection **`preRender`** flag appends a script at the **very end** of the bundle (after the artist's sketch, so it overrides the global `draw()`): it pre-greys the photo **once** with p5's own `GRAY` filter (identical tone) into a capped buffer, then each frame draws that grey image plus the colour photo at the crossfade opacity, `clear()`-ing first. No `filter()` per frame, so WebKit has nothing to corrupt. **Fidelity kept:** the colour layer stays the full 4530px photo; only the one-time grey buffer is capped at 2048px (a 4530px-wide canvas exceeds iOS's max canvas dimension, and the grey is only ever shown at display res, so the cap is invisible). Motion is untouched (the speed sweep still drives `window.opacity`, 0..10). **Verified on Chromium** by patching the real served bundle and loading it: the override takes effect (p5 calls the injected `draw`, no `filter`), and both crossfade endpoints render **pixel-identical to the original** (full-colour dune and full-grey dune, side by side). The desktop archive stays verbatim (the `dropScripts` split). **Still needs Matt's iPhone** to confirm the Safari/iOS corruption is actually gone (Chromium-only preview, as always). Reversible: drop the flag, re-mirror. If it still fails on iOS, the memory issue would have to be the two full-res image copies themselves, and the next lever is capping the colour too (last resort, a visible-on-nothing downscale). **Files.** `player/src/collections.js` (`PRERENDER_HOOK`, the end-of-bundle injection, `preRender: true` on Dune Reveries). **Setup Guide unchanged.**
