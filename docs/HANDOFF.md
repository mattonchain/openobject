...
**Connected pieces (built 2026-06-16).** Beyond files, OpenObject can also show **connected artwork**: generative or hosted pieces from a curated collection (§8), rendered as a **`connected` kind** in a **sandboxed, same-origin `<iframe>`** that fills the stage. The render bundle is mirrored locally first, so it plays offline like any file (with one exception, the live piece below). Fit/Fill does not apply (the page sizes itself); where a collection composes its art in a centered inset, an optional per-collection **crop** zooms the iframe so it still reaches the panel edges (Snowfro's *"send/receive"* fills the middle 60%, so it is zoomed to fill; Azulejo Galo renders a centered square that already fills the 1:1 panel; V4w.enko's *"Perfect Everything"* and Cinzia y Gabriel's *"Pendulum"* likewise render squares that fill the 1:1 panel edge to edge; Alex Kittoe's *"As the Days Go By"* letterboxes its own portrait photo on black). A non-square piece may instead declare an **aspect**: the display sizes the iframe to that ratio and centers it, so the bare black stage forms the letterbox top and bottom, exactly as a landscape image does in Fit mode (Juicy Julio's *"Golden Lining"*, a 5:4 landscape; no painted background, no zoom-crop). A connected piece can be a **generative sketch** (Azulejo, whose motion an optional per-collection **Animate** engages on load; or V4w.enko's *"Perfect Everything"* and Cinzia y Gabriel's *"Pendulum"*, which simply loop on their own, so no Animate), a **time-aware still** that the bundle itself redraws from the viewer's local clock (Kittoe, no motion to engage, so no Animate), a **live/networked piece** that reads on-chain state at display time and animates from it (send/receive, self-animating, with an offline caveat: see §8), or an **interactive piece we auto-animate**: Golden Lining is a black-and-white to color crossfade the artist drives by mouse, which a per-collection **motion speed (0 to 10)** instead runs as a hands-free, fluid cosine sweep (0 = static full color). See §8 and §17.
...
| Connected Collections | **Supported list; Animate on where applicable, none hidden** | Curated web/on-chain collections, added by Token ID (§8). Per-collection Animate (auto-motion) or a motion-speed slider (0 to 10), shown only for collections that move, plus Hide/Unhide. |
...
### 2026-06-19: Sixth connected collection (Cinzia y Gabriel, "Pendulum"), a second Tezos piece
Added the sixth supported Connected Collection (§8): Cinzia y Gabriel's *"Pendulum"* (Tezos FA2 contract `KT1FxLAch681RsZ2UYoSNQ5S5xsgT6mFHwZq`), an EditART generative series. It introduced **no new mechanism**: it is the union of two shapes already built, so the only change is one registry entry in `player/src/collections.js`.
...
- **Single-piece enablement (`fixedToken`).** The "Pendulum" contract holds three editions (#0 to #2), all the same sketch; the owner enables just their one piece, so the entry carries a `fixedToken` of `1` and the add flow skips the Token-ID prompt (still resolved on-chain for faithfulness), the same shape as *"Golden Lining"*.
...
- **Seeded with token 1.** Added *"Pendulum #1"* as the first piece (`m0=0.198, m1=0.675, m2=0.980, m3=0.422, m4=0.192`).
- **Verified on Mac** (Phase 1): the piece resolves via TzKT with `fixedToken`, mirrors byte-for-byte (beacon stripped, no hook injected), caches its `displayUri` thumbnail, and renders its seeded artwork; on `/display` the iframe and its square canvas fill the 1:1 stage edge to edge (confirmed from the DOM, since the headless screenshot tool mis-composites an iframe), matching *"Perfect Everything"*. A registry-only change (no `display.js`/`.css` edit), so it reaches the frame by ordinary self-update with no kiosk power-cycle.
