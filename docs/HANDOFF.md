...
- **Reliable kiosk cursor hide.** `cursor: none` on `/display` does not hide the pointer under cage: the CSS loads (the sleep-mark dim from the same file applies) but the Wayland compositor still draws its default cursor, so a pointer sits on the art when an input receiver stays attached. Confirmed 2026-06-14: it DOES vanish the moment the touchpad is nudged, so `cursor: none` works and cage only shows its default cursor until the first pointer event. Fix = hide that initial cursor (a blank/transparent cursor theme set via `XCURSOR_THEME` in the kiosk unit, or a synthetic pointer nudge at kiosk startup). Note this lives in the installer/kiosk setup, NOT the self-updatable player code, so it reaches a frame by re-running that part of `install.sh`, not via a normal Check for updates.
