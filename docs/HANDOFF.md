...
| Connected Collections | **Supported list; Animate on where applicable, none hidden** | Curated web/on-chain collections, added by Token ID (§8). Per-collection Animate (auto-motion) or a motion-speed slider (0 to 10), shown only for collections that move, plus Hide/Unhide. Experimental. |
...
- **Verified on Mac** (Phase 1): token 1 resolves on-chain via `uri()`, mirrors fully offline (no cdnjs/ipfs refs left, p5 + JPEG local), the cosine sweep runs at the chosen speed, and the piece renders edge-to-edge in width with native black bars top and bottom on the 1:1 stage. On the frame this arrives by self-update plus a kiosk power-cycle, because display.js/.css changed (§15).
