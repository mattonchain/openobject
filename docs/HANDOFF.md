...
The **Library grid is sortable** (built 2026-06-24): a **Sort** control on the Library tab orders the grid by **Recent** (newest first, the default), **Oldest**, or **Name** (A to Z, case-insensitive). The choice is a persisted server setting, and the **install sample (the Bouncing OpenObject Logo) stays anchored to the bottom in every order** (§20). This is grid presentation only; the Rotation keeps its own curated order.
