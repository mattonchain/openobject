...
### 2026-06-30: Golden Lining renders cleanly on Safari/WebKit (force a CPU-backed canvas)
Golden Lining showed gray/black tile garbage on **iPhone and Mac Safari** while rendering perfectly on the XXL. Root cause is the **browser engine**, not the screen: the sketch runs a per-frame full-canvas `filter(GRAY)` (the b&w<->colour crossfade reads the whole canvas back every frame), and WebKit tile-corrupts a GPU-backed canvas under that on a large surface; Chromium handles the same readback cleanly. The supported display surfaces are all Chromium (the XXL kiosk; the "Mac as display" path launches Google Chrome `--kiosk`), so this only bit casual Safari viewing over `http://openobject.local` and iOS (where every browser is WebKit). Matt's framing drove the fix: the platform owes the artist's **visual result**, not the literal code execution, and a corrupted render is the unfaithful outcome. Fix: a per-collection **`cpuCanvas`** flag injects a small `<head>` script (`CPU_CANVAS_PATCH`) that patches `HTMLCanvasElement.prototype.getContext` to add `willReadFrequently: true` for 2d contexts (only when unset, so an explicit choice is respected), installed before the sketch creates its canvas, forcing a CPU-backed (non-tiled) canvas. No resolution, visual, or motion change on any engine (the speed sweep and `draw()` are untouched; only the backing store moves GPU→CPU); CPU canvas is actually read-friendlier for this `filter()` workload, and the piece is a slow still-image crossfade so performance is a non-issue. Serve-side only: the desktop archive stays verbatim (same split as `dropScripts`). **Verified on Mac (Chromium):** the injection lands at the very start of `<head>` (before `p5.js`); the patch applies `willReadFrequently`, a full-canvas readback and a fill both work, and an explicit `willReadFrequently:false` is respected. **NOT yet verified on WebKit** (the preview is Chromium-only, the same blind spot that hid this twice), so the actual Safari/iOS fix is confirmed by Matt re-mirroring Golden Lining and viewing it on an iPhone; if it does not land, the stronger fallback is to replace the per-frame `filter(GRAY)` with a pre-rendered grey image + alpha crossfade (avoids the readback entirely, same visual and motion). Reversible: drop the flag, re-mirror. **Files.** `player/src/collections.js` (`CPU_CANVAS_PATCH`, the `<head>` injection, `cpuCanvas: true` on Dune Reveries). **Setup Guide unchanged.**
