...
**Connected pieces (experimental, built 2026-06-16).** Beyond files, OpenObject can also show **connected artwork**: generative or hosted pieces from a curated collection (§8), rendered as a **`connected` kind** in a **sandboxed, same-origin `<iframe>`** that fills the stage. The render bundle is mirrored locally first, so it plays offline like any file. Fit/Fill does not apply (the page sizes itself: Azulejo Galo renders a centered square that fills the 1:1 panel; Alex Kittoe's *"As the Days Go By"* letterboxes its own portrait photo on black). A connected piece is one of two kinds: a **generative sketch** whose motion an optional per-collection **Animate** engages on load, or a **time-aware still** that the bundle itself redraws from the viewer's local clock (no motion to engage, so no Animate). See §8 and §17.
